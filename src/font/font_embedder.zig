...
/// Embed an OpenType CFF font in the PDF.
/// Creates the necessary PDF objects: Type 0 font, CIDFontType0, font descriptor,
/// CFF font stream (FontFile3), ToUnicode CMap, and widths array.
pub fn embedOpenTypeFont(
    allocator: Allocator,
    store: *ObjectStore,
    font: *const OpenTypeFont,
    used_chars: []const u32,
) !EmbeddedFont {
    const tag = "ZPDFAA";
    const base_font_name = try std.fmt.allocPrint(allocator, "{s}+{s}", .{ tag, font.postscript_name });
    ...
    // 1. Font stream (CFF data embedded as FontFile3)
    const stream_ref = try store.allocate();
    const cff_raw = font.cff_data orelse return error.MissingTable;
    const stream_data = try allocator.dupe(u8, cff_raw);
    {
        var stream_dict: std.StringHashMapUnmanaged(types.PdfObject) = .{};
        try stream_dict.put(allocator, "Length", types.pdfInt(@intCast(cff_raw.len)));
        try stream_dict.put(allocator, "Subtype", types.pdfName("CIDFontType0C"));
    ...
    const desc_ref = try store.allocate();
    {
        ...
        var flags: i64 = 32;
        if (font.is_fixed_pitch) flags |= 1;
        if (font.italic_angle != 0) flags |= 64;
        try dict.dict_obj.put(allocator, "Flags", types.pdfInt(flags));
        ...
        try dict.dict_obj.put(allocator, "FontFile3", types.pdfRef(stream_ref.obj_num, stream_ref.gen_num));

        store.put(desc_ref, dict);
    }

    // 3. Build widths array
    const widths_data = try buildOtWidthsArray(allocator, font, used_chars);

    // 4. ToUnicode CMap
    const tounicode_ref = try store.allocate();
    const cmap_data = try buildOtToUnicodeCmap(allocator, font, used_chars);
    {
        var stream_dict: std.StringHashMapUnmanaged(types.PdfObject) = .{};
        try stream_dict.put(allocator, "Length", types.pdfInt(@intCast(cmap_data.len)));
    ...
    // 5. CIDFont dictionary (CIDFontType0 for CFF)
    const cidfont_ref = try store.allocate();
    {
        var dict = types.pdfDict(allocator);
        try dict.dict_obj.put(allocator, "Type", types.pdfName("Font"));
        try dict.dict_obj.put(allocator, "Subtype", types.pdfName("CIDFontType0"));
        try dict.dict_obj.put(allocator, "BaseFont", types.pdfName(base_font_name));
...
fn buildOtWidthsArray(
    ...
    for (used_chars) |cp| {
        if (font.getGlyphIndex(cp)) |gid| {
            const width = font.getGlyphWidth(gid);
            const scaled_width: i64 = @intFromFloat(@as(f64, @floatFromInt(width)) * scale);
            try writer.print("{d} [{d}] ", .{ gid, scaled_width });
        }
    }
...
test "embed OpenType CFF font creates PDF objects" {
    const opentype_mod = @import("opentype.zig");
    const font_data = try opentype_mod.buildMinimalTestOtf(std.testing.allocator);
    defer std.testing.allocator.free(font_data);
    ...
    const used_chars = [_]u32{0x41};
    var result = try embedOpenTypeFont(std.testing.allocator, &store, &font, &used_chars);
    defer result.deinit();
